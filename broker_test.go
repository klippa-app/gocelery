...
func TestBrokerRedisSend(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		messageList, err := tc.broker.Client.BRPop(tc.broker.Context(), time.Second, tc.broker.queueName).Result()
...
func TestBrokerRedisGet(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		_, err = tc.broker.Client.LPush(tc.broker.Context(), tc.broker.queueName, jsonBytes).Result()
		if err != nil {
